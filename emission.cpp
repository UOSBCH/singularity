...
money_t emission_calculator::calculate(money_t total_emission, activity_period& period)
{
    ...
    if (new_activity > emission_state.last_activity) {
        emission_state.target_emission += static_cast<money_t> (double_type(current_parameters.emission_scale) * (new_activity - emission_state.last_activity));
        ...
        return static_cast<money_t> (double_type(current_supply) * emission_limit * tanh(current_parameters.delay_koefficient * argument / emission_limit));
    } else {
